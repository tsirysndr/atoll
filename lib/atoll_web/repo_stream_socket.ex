defmodule AtollWeb.RepoStreamSocket do
  ...
  def init({:ok, cursor}) do
    %{floor: floor, latest: latest} = EventRetention.bounds()

    cond do
      cursor && cursor > latest ->
        stop("FutureCursor", "Cursor is ahead of the stream", %{})

      cursor && cursor != 0 && cursor < floor ->
        outdated(%{cursor: cursor, idle: 0}, floor)

      true ->
        {:ok, schedule(%{cursor: if(cursor == 0, do: floor, else: cursor || latest), idle: 0}, 0)}
    end
  end
  ...
  defp drain(state) do
    if Events.backlog_exceeded?(state.cursor) do
      ...
      case Events.next_frame(state.cursor) do
        ...
        {:error, :outdated_cursor} ->
          outdated(state, EventRetention.bounds().floor)
  ...
  defp outdated(state, floor) do
    frame =
      EventEncoder.info(
        "OutdatedCursor",
        "Requested history has expired; replay resumes at the retained boundary"
      )

    {:push, {:binary, frame}, schedule(%{state | cursor: floor, idle: 0}, 0)}
  end
