defmodule Atoll.Repositories.EventRetentionWorker do
  @moduledoc "Opt-in supervised bounded event retention."
  use GenServer
  ...
  def init(opts) do
    ...
    state = %{
      ...
      timer: nil,
      deadline: nil,
      supervisor: Keyword.get(opts, :task_supervisor, Atoll.Repositories.RetentionTaskSupervisor),
  ...
  defp finish(state, result, counts) do
    ...
    :telemetry.execute([:atoll, :events, :retention], Map.put(counts, :runs, 1), %{
      result: result
    })
