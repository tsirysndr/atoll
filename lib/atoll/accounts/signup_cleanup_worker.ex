defmodule Atoll.Accounts.SignupCleanupWorker do
  @moduledoc "Opt-in scheduled deletion of expired, unsubmitted signup reservations."
  use GenServer

  def children(config \\ Application.get_env(:atoll, :signup_cleanup, [])) do
    if Keyword.get(config, :enabled, false) do
      [
        Supervisor.child_spec({Task.Supervisor, name: Atoll.Accounts.SignupCleanupTaskSupervisor},
          id: Atoll.Accounts.SignupCleanupTaskSupervisor
        ),
        {__MODULE__, [interval: Keyword.get(config, :interval_ms, 3_600_000)]}
      ]
    else
      []
    end
  ...
  def init(opts) do
    ...
    state = %{
      ...
      timer: nil,
      deadline: nil,
      supervisor: Keyword.get(opts, :task_supervisor, Atoll.Accounts.SignupCleanupTaskSupervisor),
  ...
  defp run_batch do
    config = Application.get_env(:atoll, :signup_cleanup, [])

    Atoll.Accounts.SignupCleanup.batch(
      Keyword.get(config, :days, 7),
      Keyword.get(config, :limit, 100),
      false,
      DateTime.utc_now(),
      "system"
    )
  ...
  defp finish(state, result, counts) do
    ...
    :telemetry.execute(
      ...
      counts |> Map.take([:selected, :deleted]) |> Map.put(:runs, 1),
      %{result: result, more: Map.get(counts, :more, false)}
    )
