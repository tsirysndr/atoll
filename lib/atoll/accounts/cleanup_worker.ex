defmodule Atoll.Accounts.CleanupWorker do
  @moduledoc "Opt-in bounded cleanup of expired sessions and service-token replay markers."
  use GenServer
  alias Atoll.Accounts.{ServiceTokens, SessionCleanup}
  ...
  def init(opts) do
    ...
    state = %{
      ...
      timer: nil,
      deadline: nil,
      supervisor: Keyword.get(opts, :task_supervisor, Atoll.Accounts.CleanupTaskSupervisor),
  ...
  defp finish(state, result, counts) do
    ...
    :telemetry.execute([:atoll, :accounts, :cleanup], Map.put(counts, :runs, 1), %{result: result})
