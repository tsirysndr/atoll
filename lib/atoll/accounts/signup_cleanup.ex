defmodule Atoll.Accounts.SignupCleanup do
  ...
  def batch(days \\ 7, limit \\ 100, dry_run \\ true, now \\ DateTime.utc_now())

  def batch(days, limit, dry_run, %DateTime{} = now)
    ...
    Repo.transaction(fn ->
      ...
      unless dry_run do
        for row <- page do
          ...
          Atoll.Moderation.Audit.signup_cleanup!(row, cutoff)

          case Atoll.Accounts.Deletion.admin_delete(%{"did" => row.did}) do
            {:ok, :deleted} -> :ok
  ...
  def batch(_, _, _, _), do: {:error, :invalid_cleanup_options}
end
