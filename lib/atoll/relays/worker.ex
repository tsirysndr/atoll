defmodule Atoll.Relays.Worker do
  @moduledoc "Opt-in supervised periodic crawl announcements to configured relays."
  use GenServer
  ...
  def init(opts) do
    ...
    state = %{
      ...
      timer: nil,
      deadline: nil,
      supervisor: Keyword.get(opts, :task_supervisor, Atoll.Relays.TaskSupervisor),
      run: Keyword.get(opts, :run, &run_batch/0),
      interval:
        Keyword.get(
          opts,
          :interval,
          Application.get_env(:atoll, :relay_crawl_interval_seconds, 900) * 1000
        ),
  ...
  defp run_batch do
    with {:ok, results} <-
           Atoll.Relays.request_crawl(Application.get_env(:atoll, :relay_request_options, [])) do
      counts = Enum.frequencies_by(results, & &1.outcome)
      {:ok, Map.merge(%{accepted: 0, host_banned: 0, unavailable: 0, rejected: 0}, counts)}
    end
  end
  ...
  defp finish(state, result, counts) do
    ...
    :telemetry.execute([:atoll, :relay, :announcement], Map.put(counts, :runs, 1), %{
      result: result
    })
