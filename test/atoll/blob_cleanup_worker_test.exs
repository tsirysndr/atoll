defmodule Atoll.BlobCleanupWorkerTest do
  use Atoll.DataCase, async: false
  alias Atoll.Blobs.{Blob, CleanupWorker}
  ...
  setup do
    ...
    owner = self()
    handler = make_ref()
    :ok = :telemetry.attach(handler, [:atoll, :blobs, :cleanup], &__MODULE__.report/4, owner)
  ...
  test "scheduled batch expires and collects real PostgreSQL blobs", c do
    did = "did:plc:scheduledcleanup"
    {:ok, _} = Repositories.create(did, SigningKey.generate())
    {:ok, _} = Blobs.stage(did, "expired", "text/plain")
    cid = CID.create("expired", :raw)
    Repo.update_all(Blob, set: [staged_at: DateTime.add(DateTime.utc_now(), -172_800)])
    worker = worker(c)
    tick(worker)
    assert_receive {:cleanup, :ok, %{expired: 1, deleted: 1, runs: 1}}, 1000
  ...
  test "manual triggers do not overlap and a finished run schedules its successor", c do
    ...
    send(task, :finish)
    assert_receive {:cleanup, :ok, %{deleted: 2, runs: 1}}
    tick(worker)
